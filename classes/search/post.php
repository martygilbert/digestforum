...
class post extends \core_search\base_mod {
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        list ($contextjoin, $contextparams) = $this->get_context_restriction_sql(
                $context, 'digestforum', 'f');
        if ($contextjoin === null) {
            return null;
        }

        $sql = "SELECT fp.*, f.id AS digestforumid, f.course AS courseid, fd.groupid AS groupid
    ...
    public function attach_files($document) {
        ...
        $cm = $this->get_cm($this->get_module_name(), $post->digestforum, $document->get('courseid'));
        $context = \context_module::instance($cm->id);
    ...
    /**
     * Changes the context ordering so that the digestforums with most recent discussions are indexed
     * first.
     *
    ...
    protected function get_contexts_to_reindex_extra_sql() {
        return [
            'JOIN {digestforum_discussions} fd ON fd.course = cm.course AND fd.digestforum = cm.instance',
