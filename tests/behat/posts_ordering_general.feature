...
Feature: New discussions and discussions with recently added replies are displayed first
  ...
  Scenario: Replying to a digestforum post or editing it puts the discussion to the front
    ...
    And I follow "Course general digestforum"
    #
    # Make sure the order of the digestforum posts is as expected (most recently participated first).
    #
    Then I should see "Forum post 3" in the "//tr[contains(concat(' ', normalize-space(@class), ' '), ' discussion ')][position()=3]" "xpath_element"
    And I should see "Edited digestforum post 2" in the "//tr[contains(concat(' ', normalize-space(@class), ' '), ' discussion ')][position()=2]" "xpath_element"
    ...
    #
    And I follow "Edited digestforum post 2"
